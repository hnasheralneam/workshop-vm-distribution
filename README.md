...
## Admin portal
The admin page provides a web UI for provisioning and destroying VMs. You can set the `ADMIN_PASSWORD` in `.env`. Saved pools are listed as rows in a single panel, sorted by most recent use or edit, and organized under tabs (all, workshop, challenge, generic) with per-tab counts. The search bar and filters (all, in use, free, dispenser, private) apply within the active tab, and typing in the search box jumps to the all tab. Each row lines up the pool name, claim code (click to copy), OS, group, in-use/total counts, a claim-link copy button and edit button that appear on hover, a lock that toggles the pool between public and private, a delete button, and a one-click Deploy with a VM count (blue for Windows pools, green for Linux). On touch screens the buttons are always visible and the rows stack.

You create pools with the New pool dialog: a name and group, the template (access method, VM ID, username, password), and the claim behavior (VM count, claim validity in hours, Dispenser and Private switches). A dispenser pool provisions a fresh VM whenever someone claims from it and the pool has none free. A private pool is hidden from the main page and can only be claimed with its code. The same dialog renames, reconfigures, or deletes an existing pool. Every pool is assigned a 5-character claim code automatically; the code is the pool's permanent ID, so claim links keep working and pool names can be changed at any time without touching existing VMs.
